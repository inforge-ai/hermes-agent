...
_PLATFORMS = [
    ...
    },
    {
        ...
        "setup_instructions": [
            "1. In Zulip: Personal Settings → Bots → Add a new bot",
            "2. Choose 'Generic bot' and copy the bot email + API key",
            "3. Subscribe the bot to the streams where you want it to respond",
            "4. To find your user ID: GET /api/v1/users/me, or use the bot admin UI",
            "5. Home channel is a stream_id:topic pair (set later with /sethome if unsure)",
        ],
        "vars": [
            {"name": "ZULIP_SITE", "prompt": "Zulip server URL (e.g. https://zulip.example.com)", "password": False,
             "help": "Your Zulip server URL. Works with any self-hosted instance or Zulip Cloud."},
            {"name": "ZULIP_EMAIL", "prompt": "Bot email (e.g. hermes-bot@zulip.example.com)", "password": False,
             "help": "Paste the bot email from step 2 above."},
            {"name": "ZULIP_API_KEY", "prompt": "Bot API key", "password": True,
             "help": "Paste the bot API key from step 2 above."},
            {"name": "ZULIP_ALLOWED_USERS", "prompt": "Allowed user IDs (comma-separated)", "password": False,
             "is_allowlist": True,
             "help": "Zulip user IDs (numeric) allowed to interact with the bot."},
            {"name": "ZULIP_HOME_STREAM", "prompt": "Home stream ID (or empty to set later with /sethome)", "password": False,
             "help": "Stream ID where Hermes delivers cron results and notifications."},
            {"name": "ZULIP_HOME_TOPIC", "prompt": "Home topic name (default: notifications)", "password": False,
             "help": "Topic within the home stream for cron deliveries."},
        ],
    },
    {
        "key": "whatsapp",
